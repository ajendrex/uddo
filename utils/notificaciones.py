...
remitente = 'no-responder@uddo.unab.cl'
...
def notificarCreacionCurso(request, curso):
  ...
  send_mail(asuntoCursoCreado, msg, remitente, [di.email], fail_silently=False)
  ...
  send_mail(asuntoCursoCreado, msg, remitente, [profesor.email], fail_silently=False)
...
def notificarCreacionRecurso(request, recurso):
  ...
  send_mail(asuntoRecursoCreado, msg, remitente, [di.email], fail_silently=False)
...
def notificarProveedorAsignado(request, recurso, proveedorAnterior):
  ...
  if proveedorAnterior:
    ...
    send_mail(asuntoProveedorAsignado, msg, remitente, [proveedorAnterior.email], fail_silently=False)
    ...
    send_mail(asuntoProveedorAsignado, msg, remitente, [di.email], fail_silently=False)
  if proveedor:
    ...
    send_mail(asuntoProveedorAsignado, msg, remitente, [proveedor.email], fail_silently=False)
